...
combined <- read.csv("OLD Data Sets/Combined file from both BSB venting descending studies updated June 16 2021_HS.csv")
head(combined)
dim(combined)
...
exp1$surfaceTemp = as.numeric(exp1$SurfaceWaterTempC_centered)
exp1$bottomTemp = as.numeric(exp1$BottomWaterTempC_centered)
exp1$gradientTemp = as.numeric(exp1$Gradient_Temp_centered)
exp1$TL = as.numeric(exp1$TL_mm_centered)
...
exp2$TL = as.numeric(exp2$TL_mm_centered)
exp2$gradientTemp = as.numeric(exp2$Gradient_Temp_centered)
exp2$surfaceTemp = as.numeric(exp2$SurfaceWaterTempC_centered)
